<template>
  <div class="role-wrapper">
    <div class="handle-wrap">
      <a-space>
        <a-button type="primary" @click="handleAdd">新增</a-button>
      </a-space>
      <a-space />
    </div>
    <div class="table-wrap">
      <ym-table
        rowKey="deptId"
        :columns="deptColumns"
        :getTableList="getDeptList"
        :rowSelection="false"
        ref="tableInstance"
      >
        <template #slotOne="{ record }">{{ parseDicts(record.status) }}</template>
        <template #action="{ record }">
          <a-space>
            <a-button type="link" size="small" @click="handleEdit(record)">修改</a-button>
            <a-button type="link" size="small" danger @click="handleDel(record.deptId)">删除</a-button>
          </a-space>
        </template>
      </ym-table>
    </div>
    <dept-detail ref="detailInstance" :getSourceData="getSourceData" :handleRefresh="handleRefresh" />
  </div>
</template>

<script setup lang="ts">
import { ref, onBeforeMount } from 'vue'
import YmTable from '@/components/common/YmTable.vue'
import DeptDetail from '@/components/setting/DeptDetail.vue'
import { apiGetDeptList, apiDeleteDept } from '@/service/api/setting'
import { apiGetDicts } from '@/service/api/common'
import { deptColumns } from '@/enums/tableHeadEnum'
import { actionTypeEnum } from '@/enums/commonEnum'
import { message } from 'ant-design-vue'
import { YmModal } from '@/utils/antd'

onBeforeMount(() => {
  getDicts()
})

/**
 ********************************* 表格逻辑 ******************************************
 */

// 表格实例
const tableInstance = ref()

// 详情实例
const detailInstance = ref()

// 部门状态
const deptStatusOptions = ref<any>([])

/**
 * @desc 列表刷新
 */
const handleRefresh = () => {
  tableInstance.value.handleRefresh()
}

/**
 * @desc 重新获取当前页列表
 */
const getSourceData = () => {
  tableInstance.value.getSourceData()
}

/**
 * @desc 获取用户列表
 */
const getDeptList = async () => {
  const { current, size } = tableInstance.value.reqParams
  const { code, data, total } = await apiGetDeptList({
    pageSize: size,
    pageNum: current,
  })
  return Promise.resolve({
    code,
    data: {
      records: arrayToTree(data, data[0].parentId),
      total,
    },
  })
}

/**
 * @desc 一维数组转为树结构
 */
const arrayToTree = (arr: any[], pid: number) => {
  return arr.reduce((res, current) => {
    if (current['parentId'] === pid) {
      current.children = arrayToTree(arr, current['deptId'])
      return res.concat(current)
    }
    return res
  }, [])
}

/**
 * @desc 获取部门状态字典
 */
const getDicts = async () => {
  const { code, data } = await apiGetDicts({ typeKey: 'sys_normal_disable' })
  if (code === 20000) {
    deptStatusOptions.value = data
  }
}

/**
 * @desc 解析字典内容
 */
const parseDicts = (key: any) => {
  const findItem = deptStatusOptions.value.find((item: any) => item.dictValue === key)
  return findItem ? findItem.dictLabel : '--'
}

/**
 * @desc 列表删除
 */
const handleDel = (deptId: number) => {
  YmModal.delete(async () => {
    const { code } = await apiDeleteDept({ deptId })
    if (code === 20000) {
      message.success('删除成功')
      getSourceData()
    }
  })
}

/**
 * @desc 新增
 */
const handleAdd = () => {
  detailInstance.value.initModal(actionTypeEnum.ADD)
}

/**
 * @desc 编辑
 */
const handleEdit = (record: any) => {
  detailInstance.value.initModal(actionTypeEnum.EDIT, record)
}
</script>

<style lang="less" scoped>
.role-wrapper {
  .handle-wrap {
    display: flex;
    justify-content: space-between;
    margin-bottom: 14px;
  }
}
</style>
